...
class ComparativeAnalysis:
    ...
    def __init__(self, geometry: ManifoldRingChannelGeometry = None, tp_fluid: str = "HFE7100"):
        self.geo = geometry or ManifoldRingChannelGeometry()
        self.sp_sim = SinglePhaseSimulation(self.geo, FluidProperties("water"))
        self.tp_sim = TwoPhaseSimulation(self.geo, FluidProperties(tp_fluid))
    ...
    def sweep_comparison(self,
        ...
        if sp_flow_gs is None:
            sp_flow_gs = flow_rate_gs
        if tp_flow_gs is None:
            tp_flow_gs = flow_rate_gs

        results = []
        for qf in heat_flux_range:
            cp = self.compare_at_condition(qf, sp_flow_gs, tp_flow_gs, T_inlet)
            results.append(cp)
        return results
    ...
    def generate_report(self,
        ...
        tp_name = self.tp_sim.fluid.fluid_name

        lines = [
            "=" * 70,
            f"仿荷叶歧管微通道冷板: 单相水冷 vs 两相{tp_name}沸腾冷却 对比分析",
            "=" * 70,
            "",
        ...
        for qf in heat_flux_list:
            cp = self.compare_at_condition(qf, sp_flow_gs, tp_flow_gs)
            lines.append(
        ...
        lines.extend([
            ...
            "",
            "关键结论:",
            f"1. 跨工质对比: 单相水冷因水极其优异的物性, 其换热系数不同于两相{tp_name}沸腾",
            f"2. 同工质对比: 两相沸腾(利用相变潜热)换热系数远高于同工质单相对流",
            f"3. 运行壁温: {tp_name}在饱和温度({self.tp_sim.fluid.T_sat:.1f}°C)下发生相变, 适用于等温相变冷却场景",
