...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"单相水冷 @ q={args.heat_flux} W/cm², m={args.flow_rate} g/s")
    print(f"{'='*60}")
    print(f"  散热功率:   {res_sp.Q_total:.1f} W")
    print(f"  换热系数:   {res_sp.h_conv_cm2:.3f} W/(cm²·K)")
    print(f"  最高壁温:   {res_sp.T_wall_max:.1f} °C")
    print(f"  压降:       {res_sp.pressure_drop/1e3:.2f} kPa")
    print(f"  热阻:       {res_sp.thermal_resistance:.4f} (cm²·K)/W")
    ...
    print(f"\n{'='*60}")
    print(f"两相HFE-7100 @ q={args.heat_flux} W/cm², m={args.flow_rate} g/s")
    print(f"{'='*60}")
    print(f"  散热功率:   {res_tp.Q_total:.1f} W")
    print(f"  换热系数:   {res_tp.h_conv_cm2:.3f} W/(cm²·K)")
    print(f"  最高壁温:   {res_tp.T_wall_max:.1f} °C")
    print(f"  压降:       {res_tp.pressure_drop/1e3:.2f} kPa")
    print(f"  热阻:       {res_tp.thermal_resistance:.4f} (cm²·K)/W")
    print(f"  COP:        {res_tp.COP:.0f}")
    print(f"  CHF:        {res_tp.CHF:.1f} W/cm²")
